...
- [ ] **Define the core problem your app solves**
  - [ ] Write down the problem in one sentence, from the user's perspective
  - [ ] Write your one-sentence value proposition ("X helps Y do Z without W")
  - [ ] List 3 ways people solve this problem today (including "they don't")
  - [ ] Explain in one paragraph why your solution is better/faster/cheaper
  - [ ] Test the pitch on 5 people outside your circle; note whether they "get it" in under 10 seconds

- [ ] **Identify your target audience**
  - [ ] Define 1–2 primary user personas (age, occupation, tech comfort, motivation)
  ...
  - [ ] Determine willingness to pay — look at what they already pay for adjacent solutions
...
- [ ] **Competitive research**
  ...
  - [ ] Write down 2–3 gaps you'll exploit (features, UX, price, niche)
  - [ ] Save competitor screenshots for design reference

- [ ] **Define MVP feature set**
  - [ ] Brainstorm every feature you can imagine (no filtering yet)
  - [ ] Sort into three buckets: Must-have (v1), Should-have (v1.x), Someday
  - [ ] Apply the test: "Can the app deliver its core value without this?" If yes → cut from v1
  - [ ] Target 3–5 must-have features maximum
  - [ ] Write a one-line "definition of done" for each must-have feature
...
- [ ] **Choose monetization model**
  ...
  - [ ] Decide: free, paid up front, freemium, subscription, ads, or in-app purchases
  - [ ] If subscription: pick tiers and price points (check App Store price tiers)
  - [ ] If freemium: define exactly which features are free vs. paid
  - [ ] Factor in Apple's 15–30% commission when setting prices
  - [ ] Decide whether to offer a free trial and its length

- [ ] **Sketch user flows**
  - [ ] List every screen the MVP needs (login, home, settings, paywall, etc.)
  - [ ] Draw the "happy path" — from app open to core value delivered — in as few steps as possible
  - [ ] Map onboarding flow (first launch → signed up → first success moment)
  - [ ] Map edge flows: forgot password, empty states, no internet, purchase failure
  - [ ] Number the screens and draw arrows between them (paper or Figma/FigJam)
...
- [ ] **Check compliance needs**
  ...
  - [ ] HIPAA: only if handling protected health info on behalf of covered entities — most consumer health apps aren't, but verify
...
- [ ] **Wireframes**
  ...
  - [ ] Create a frame for every screen from your Phase 1 flow map
  - [ ] Lay out each screen with boxes and placeholder text only — no colors, no polish
...
- [ ] **Follow Apple's Human Interface Guidelines (HIG)**
  ...
  - [ ] Use standard iOS components (tab bars, navigation stacks, sheets) unless you have a strong reason not to
...
- [ ] **Website design**
  ...
  - [ ] Mock simple layouts for privacy, terms, and support pages
...
- [ ] **Choose your tech stack**
  - [ ] Native Swift + SwiftUI if: iOS-only for now, want best performance and fastest access to new iOS features
  - [ ] Flutter or React Native if: Android version is definitely coming and you want one codebase
  - [ ] Decide your minimum iOS version (supporting the last 2 major versions covers ~90%+ of devices)
  - [ ] Pick your architecture pattern up front (e.g., MVVM) and stick to it

### Build

- [ ] **Set up version control**
  - [ ] Initialize a Git repo in the project; add a Swift/Xcode .gitignore
  - [ ] Create a private GitHub/GitLab repo and push
  - [ ] Commit small and often with descriptive messages
  - [ ] Never commit secrets (API keys) — use a gitignored config file or environment values
  - [ ] Optional: set up a CI pipeline (GitHub Actions/Xcode Cloud) to build on every push

- [ ] **Build core MVP features**
  - [ ] Break each must-have feature into tasks in a tracker (GitHub Issues, Linear, Trello)
  - [ ] Build the app skeleton first: navigation structure + empty screens matching your designs
...
- [ ] **Implement authentication**
  ...
  - [ ] Choose provider: Sign in with Apple, email/password, Google, etc. (Firebase Auth/Supabase Auth handle the plumbing)
...
- [ ] **Decide if you need a backend at all**
  - [ ] No accounts, no sync, no shared data → you may not need one (store locally / iCloud)
  - [ ] Need auth, sync, or server logic → pick Option A or B below
...
- [ ] **Set up the database**
  ...
  - [ ] Create separate dev and production databases
  - [ ] Restrict network access to your app servers only; strong credentials in a secrets manager

- [ ] **Build the API**
  - [ ] Pick a framework you already know (Node/Express, Python/FastAPI, Go, etc.)
  - [ ] Define endpoints from your app's actual screens — don't build speculative endpoints
  - [ ] Implement auth middleware (JWT validation) on every protected route
  - [ ] Validate all input server-side; never trust the client
  - [ ] Add rate limiting on auth and expensive endpoints
  - [ ] Version the API (/v1/) so future changes don't break shipped apps
  - [ ] Write basic API docs, even just a README
...
- [ ] **Set up monitoring/alerts**
  ...
  - [ ] Add a /health endpoint the monitor can hit
  - [ ] Set up log retention and error alerting (host dashboards, or Sentry for the API)
  - [ ] Add disk/memory alerts on VPS instances

- [ ] **Plan for scaling (lightly)**
  - [ ] Keep the app server stateless (sessions in the DB/token, files in object storage) so you can add servers later
  - [ ] Add database indexes for your common queries
  - [ ] Write down your "if we get popular" plan in one paragraph — then stop; don't build it yet
...
- [ ] **Choose how to build it**
  ...
  - [ ] Set up the project/site and connect your custom domain
